...
def test_ind_large(model, predictor, evaluator, batch_size):
    ...
    adj_t = adj.t().to(device)
    data.x = (adj_t.sum(dim=0) / (4267 - 427)).reshape(-1, 1)
    x = data.x
    pos_test_edge = split_edge['test']['edge'].to(x.device)
    neg_test_edge = split_edge['test']['edge_neg'].to(x.device)
...
def main(seed1, seed2):
    ...
    if args.method == 'SAGE':
        model = SAGE(1, args.hid_dim, args.hid_dim, args.num_layers, 0.5, aggr=args.agg).to(device)
    elif args.method == 'GCN':
        model = GCN(1, args.hid_dim, args.hid_dim, args.num_layers, 0.5).to(device)
    elif args.method == 'GAT':
        model = GAT(1,args.hid_dim, args.hid_dim, num_layer=args.num_layers, aggr=args.agg).to(device)
    else:
    ...
    best_valid_acc = 0
    valid_losses = []
    checkpoint_file_name = "7-30-ppa" + str(args.agg) + str(args.hid_dim)+str(args.num_layers)+ str(args.num_nodes) + str(args.method) + str(args.noinn) + "model_GNN_checkpoint.pth.tar"
    checkpoint_file_name_Y ="7-30-ppa" + str(args.agg) + str(args.hid_dim)+str(args.num_layers)+ str(args.num_nodes) + str(args.method) + str(args.noinn) + "model_link_checkpoint.pth.tar"
    n = 1
    ...
    results_ind = test_ind(model, predictor, evaluator, 1280)
    print(results_ind)

    results_ind_1 = test_ind_large(model, predictor, evaluator, 1280)
    print(results_ind_1)
