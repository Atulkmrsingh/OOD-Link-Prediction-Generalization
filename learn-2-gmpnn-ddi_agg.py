...
class jgMPNN(torch.nn.Module):
    def __init__(self, in_channels=1, hidden_channels=5, out_channels=1):
        ...
        self.nn_agg = torch.nn.Linear(in_channels + in_channels, out_channels)
    ...
    def reset_parameters(self):
        self.nn_agg.reset_parameters()
        self.nn1.reset_parameters()
        self.nn2.reset_parameters()

    def forward(self, a, num_nodes, f, de_2):
        f = f.reshape(-1, 1)
        f = self.nn_agg(torch.cat((f, f), dim=1)).reshape(num_nodes, num_nodes)
...
def train(model, predictor, f, a, de_2, split_edge, edge_index, optimizer, batch_size):
    ...
    margin = 0.2  # Set your margin hyperparameter
    ...
    for perm in DataLoader(range(pos_train_edge.size(0)), batch_size, shuffle=True):
        ...
        edge_neg = negative_sampling(edge_index, num_nodes=427, num_neg_samples=perm.size(0), method='dense')
        neg_out = f_new[edge_neg[0], edge_neg[1]]
        neg_loss = -torch.log(1 - neg_out + 1e-15).mean()

        # Margin Ranking Loss
        loss = F.margin_ranking_loss(pos_out, neg_out, target=torch.ones_like(pos_out), margin=margin)
