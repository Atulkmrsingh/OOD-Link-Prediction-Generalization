...
class jgMPNN(torch.nn.Module):
    def __init__(self, in_channels=1, hidden_channels=5, out_channels=1):
        ...
        self.nn_agg_hid = torch.nn.Linear(2*hidden_channels, hidden_channels)
    ...
    def forward(self, a, num_nodes, f, de_2):
        ...
        if self.hid > 1:
            ...
            f = self.nn_agg_hid(torch.cat((f, f), dim=1)).reshape(num_nodes, num_nodes, self.hid)
            a = a.to_dense()
...
def test_ind(model, predictor, length, n, evaluator, batch_size, seed):
    # Inductive
    print("Starting Inductive test for n",n)
    ...
    print("Running model for Inductive test for n", n)
    f_new = model(a, num_nodes, f, de_2)
    f_new = predictor(f_new.reshape(-1, 1)).reshape(num_nodes, num_nodes)
    pos_test_preds = []
    for perm in DataLoader(range(pos_test_edge.size(0)), batch_size):
