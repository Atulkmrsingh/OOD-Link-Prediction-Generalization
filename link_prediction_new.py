...
class GAT(torch.nn.Module):
    def __init__(self, in_channels=1, hid_channels=10, out_channels=10, num_layer=2):
        ...
        if self.num_layer > 2:
            ...
            self.conv3 = GATConv(self.hid * self.in_head, self.hid, heads=self.in_head)#, dropout=0.5)
        self.bn1 = torch.nn.BatchNorm1d(self.hid * self.in_head)
        self.conv1 = GATConv(in_channels, self.hid, heads=self.in_head)#, dropout=0.5)
...
def main(seed1, seed2):
    ...
    if args.method == 'SAGE':
        model = SAGE(1, args.hid_dim, args.hid_dim, args.num_layers, 0).to(device)
    elif args.method == 'GCN':
        model = GCN(1, args.hid_dim, args.hid_dim, args.num_layers, 0).to(device)
    elif args.method == 'GAT':
        model = GAT(1,args.hid_dim, num_layer=args.num_layers).to(device)
    else:
    ...
    checkpoint_file_name_Y ="5-15" + str(args.hid_dim)+str(args.num_layers)+ str(args.num_nodes) + str(args.method) + str(args.noinn) + "model_link_checkpoint.pth.tar"
