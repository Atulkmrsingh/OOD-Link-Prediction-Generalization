...
def train(model, predictor, f, a, de_2, split_edge, edge_index, optimizer, batch_size):
    ...
    for perm in DataLoader(range(pos_train_edge.size(0)), batch_size, shuffle=True):
        ...
        pos_out = f_new[pos_edge[0], pos_edge[1]].squeeze()
        pos_target = torch.ones_like(pos_out)
        
        # Negative edges (randomly sampled)
        num_neg_edge = perm.size(0)
        neg_edge = torch.randint(0, num_nodes, (num_neg_edge, 2)).to(f.device)
        neg_out = f_new[neg_edge[:, 0], neg_edge[:, 1]].squeeze()
        neg_target = torch.zeros_like(neg_out)
        
        # Combine positive and negative targets and predictions
...
def test_ind(model, predictor, length, n, evaluator, batch_size, seed):
    ...
    probs = [[0.6, 0.1, 0.03], [0.1, 0.6, 0.1], [0.03, 0.1, 0.6]]
