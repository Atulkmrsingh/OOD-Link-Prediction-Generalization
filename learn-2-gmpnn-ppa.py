...
def train(model, predictor, f, a, de_2, split_edge, edge_index, optimizer, batch_size):
    ...
    for perm in DataLoader(range(pos_train_edge.size(0)), batch_size,
        ...
        edge = negative_sampling(edge_index, num_nodes=57628,
...
def test_ind(model, predictor, evaluator, batch_size):
    ...
    idxs_small_tr = (((split_edge['train']['edge'][:, 0] < 100000).to(torch.int) + (
            split_edge['train']['edge'][:, 1] < 100000).to(torch.int) +
                      (split_edge['train']['edge'][:, 0] >= 57628).to(torch.int) + (
                              split_edge['train']['edge'][:, 1] >= 57628).to(torch.int)) == 4)
    idxs_small_te = (((split_edge['test']['edge'][:, 0] < 100000).to(torch.int) + (
            split_edge['test']['edge'][:, 1] < 100000).to(torch.int) +
                      (split_edge['test']['edge'][:, 0] >= 57628).to(torch.int) + (
                              split_edge['test']['edge'][:, 1] >= 57628).to(torch.int)) == 4)
    idxs_small_te_neg = (((split_edge['test']['edge_neg'][:, 0] < 100000).to(torch.int) + (
            split_edge['test']['edge_neg'][:, 1] < 100000).to(torch.int) +
                          (split_edge['test']['edge_neg'][:, 0] >= 57628).to(torch.int) + (
                                  split_edge['test']['edge_neg'][:, 1] >= 57628).to(torch.int)) == 4)

    split_edge['train']['edge'] = split_edge['train']['edge'][idxs_small_tr] - 57628
    split_edge['test']['edge'] = split_edge['test']['edge'][idxs_small_te] - 57628
    split_edge['test']['edge_neg'] = split_edge['test']['edge_neg'][idxs_small_te_neg] - 57628

    row, col, edge_attr = data.adj_t.t().coo()
    data.edge_index = torch.stack([row, col], dim=0)
    idx_adj = (((data.edge_index[0] < 100000).to(torch.int) + (
            data.edge_index[1] < 100000).to(torch.int) + (data.edge_index[0] >= 57628).to(torch.int) + (
                        data.edge_index[1] >= 57628).to(torch.int)) == 4)
    data.edge_index = data.edge_index[:, idx_adj] - 57628

    adj = SparseTensor(row=data.edge_index[0], col=data.edge_index[1], sparse_sizes=(57628, 57628))
...
def test_ind_large(model, predictor, evaluator, batch_size):
    ...
    # data.x = adj_t.to_dense().sum(dim=1)
    split_edge = dataset.get_edge_split()
    idxs_small_tr = (((split_edge['train']['edge'][:, 0] >= 57628).to(torch.int) + (
            split_edge['train']['edge'][:, 1] >= 57628).to(torch.int)) == 2)
    idxs_small_te = (((split_edge['test']['edge'][:, 0] >= 57628).to(torch.int) + (
            split_edge['test']['edge'][:, 1] >= 57628).to(torch.int)) == 2)
    idxs_small_te_neg = (((split_edge['test']['edge_neg'][:, 0] >= 57628).to(torch.int) + (
            split_edge['test']['edge_neg'][:, 1] >= 57628).to(torch.int)) == 2)
    idxs_small_va = (((split_edge['test']['edge'][:, 0] < 100000).to(torch.int) + (
            split_edge['test']['edge'][:, 1] < 100000).to(torch.int) +
                      (split_edge['test']['edge'][:, 0] >= 57628).to(torch.int) + (
                              split_edge['test']['edge'][:, 1] >= 57628).to(torch.int)) == 4)
    idxs_small_va_neg = (((split_edge['test']['edge_neg'][:, 0] < 100000).to(torch.int) + (
            split_edge['test']['edge_neg'][:, 1] < 100000).to(torch.int) +
                          (split_edge['test']['edge_neg'][:, 0] >= 57628).to(torch.int) + (
                                  split_edge['test']['edge_neg'][:, 1] >= 57628).to(torch.int)) == 4)

    split_edge['train']['edge'] = split_edge['train']['edge'][idxs_small_tr] - 57628
    split_edge['valid']['edge'] = split_edge['test']['edge'][idxs_small_va] - 57628
    split_edge['valid']['edge_neg'] = split_edge['test']['edge_neg'][idxs_small_va_neg] - 57628
    split_edge['test']['edge'] = split_edge['test']['edge'][idxs_small_te] - 57628
    split_edge['test']['edge_neg'] = split_edge['test']['edge_neg'][idxs_small_te_neg] - 57628
    ...
    row, col, edge_attr = data.adj_t.t().coo()
    data.edge_index = torch.stack([row, col], dim=0)
    idx_adj = (((data.edge_index[0] >= 57628).to(torch.int) + (
            data.edge_index[1] >= 57628).to(torch.int)) == 2)
    data.edge_index = data.edge_index[:, idx_adj] - 57628

    adj = SparseTensor(row=data.edge_index[0], col=data.edge_index[1], sparse_sizes=(576289	 - 57628, 576289	 - 57628))
    adj_t = adj.t().to(device)

    num_nodes = 576289	 - 57628
...
def main(seed1,seed2):
    ...
    idxs_small_tr = (((split_edge['train']['edge'][:, 0] < 57628).to(torch.int) + (
            split_edge['train']['edge'][:, 1] < 57628).to(torch.int)) == 2)
    idxs_small_va = (((split_edge['valid']['edge'][:, 0] < 57628).to(torch.int) + (
            split_edge['valid']['edge'][:, 1] < 57628).to(torch.int)) == 2)
    idxs_small_te = (((split_edge['test']['edge'][:, 0] < 57628).to(torch.int) + (
            split_edge['test']['edge'][:, 1] < 57628).to(torch.int)) == 2)
    idxs_small_va_neg = (((split_edge['valid']['edge_neg'][:, 0] < 57628).to(torch.int) + (
            split_edge['valid']['edge_neg'][:, 1] < 57628).to(torch.int)) == 2)
    idxs_small_te_neg = (((split_edge['test']['edge_neg'][:, 0] < 57628).to(torch.int) + (
            split_edge['test']['edge_neg'][:, 1] < 57628).to(torch.int)) == 2)
    ...
    data.x = (adj_t.sum(dim=0) / 57628).reshape(-1, 1)
    ...
    best_valid_acc = 0
    checkpoint_file_name = "7-30ppa" + str(args.num_nodes) + str(args.hid_dim) + str(args.num_runs) + "model_2GNN_checkpoint.pth.tar"
    checkpoint_file_name_Y ="7-30ppa" + str(args.hid_dim)+str(args.num_layers)+ str(args.num_nodes)  + "model_2GNN_link_checkpoint.pth.tar"
    n = 1
